...
namespace Tojeero.Core
{
    public class CacheRepository : ICacheRepository
    {
        ...
        readonly AsyncReaderWriterLock readerWriterLock = new AsyncReaderWriterLock();
            ...
            IProductFilter filter = null)
        {
            return
                await
                    find(searchQuery, (ITableQuery<Product> q) => q.OrderBy(p => p.LowercaseName), pageSize, offset)
            ...
            IStoreFilter filter = null)
        {
            return
                await
                    find(searchQuery, (ITableQuery<Store> q) => q.OrderBy(s => s.LowercaseName), pageSize, offset)
                        .ConfigureAwait(false);
        }
        ...
        {
            var connection =
                _factory.Create(PortablePath.Combine(_deviceContext.CacheFolder.Path, Constants.DatabaseFileName));
            return connection;
        }
        ...
        public Task<IEnumerable<T>> find<T>(string searchQuery, Action<ITableQuery<T>> orderByAction, int pageSize,
            int offset) where T : ISearchableEntity, new()
        {
            return Task<IEnumerable<T>>.Factory.StartNew(() =>
            {
                using (var source = new CancellationTokenSource(TimeSpan.FromSeconds(TIMEOUT_SECONDS)))
        ...
        private async Task deleteDatabase()
        {
            ...
            {
                Tools.Logger.Log(ex, "Error occured while deleting database from local cache.", LoggingLevel.Warning,
                    true);
            }
        }
        ...
        private bool isEntityCacheExpired(string cacheName)
        {
            using (var connection = getConnection())
            {
                ...
                var queryCache =
                    connection.Table<CachedQuery>()
                        .Where(q => q.EntityName == cacheName)
                        .OrderBy(q => q.LastFetchedAt)
                        .FirstOrDefault();
        ...
        private ITableQuery<Product> getFilteredProductQuery(ITableQuery<Product> query, IProductFilter filter,
