		...
		private async Task reloadCities()
		{
			...
			using (var writerLock = await _citiesLock.WriterLockAsync())
			{
				if (!(this.StoreFilter.Country == null || this.Countries == null || this.Countries.Length == 0) &&
					!(this.Cities != null && this.Cities.Length > 0 && this.Cities[0].CountryId == this.StoreFilter.Country.CountryId))
				{
					try
					{
						var result = await _cityManager.Fetch(this.StoreFilter.Country.CountryId);
