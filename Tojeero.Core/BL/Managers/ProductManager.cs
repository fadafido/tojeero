...
namespace Tojeero.Core.Managers
{
    public class ProductManager : IProductManager
    {
        ...
        public async Task<IEnumerable<IProduct>> Fetch(int pageSize, int offset, IProductFilter filter = null)
        {
            var products =
                await
                    _manager.Fetch<IProduct, Product>(new FetchProductsQuery(pageSize, offset, _manager, filter),
        ...
        {
            return _manager.Fetch<IProduct, Product>(new FetchFavoriteProductsQuery(pageSize, offset, _manager),
                Constants.ProductsCacheTimespan.TotalMilliseconds);
        }
        ...
        public async Task<IEnumerable<IProduct>> Find(string query, int pageSize, int offset,
            IProductFilter filter = null)
        {
            var products =
                await
                    _manager.Fetch<IProduct, Product>(new FindProductsQuery(query, pageSize, offset, _manager, filter),
                        Constants.ProductsCacheTimespan.TotalMilliseconds);
            await setCountries(products);
            return products;
        }
        ...
        public async Task<IProduct> Save(ISaveProductViewModel product)
        {
            if (product != null)
            {
                if (product.HasChanged)
                {
                    ...
                    if (result != null)
                    {
                        _messenger.Publish(new ProductChangedMessage(this, result,
                            product.IsNew ? EntityChangeType.Create : EntityChangeType.Update));
                    }
                    return result;
                }
    ...
    public class FetchProductsQuery : IQueryLoader<IProduct>
    {
        ...
        readonly IProductFilter filter;
        ...
        {
            var cachedQueryId = string.Format("products:p_{0}o_{1}-f_{2}", pageSize, offset, filter);
    ...
    public class FetchFavoriteProductsQuery : IQueryLoader<IProduct>
    {
        ...
        public override string ToString()
        {
            var cachedQueryId = string.Format("favorite_products:p_{0}o_{1}", pageSize, offset);
            return cachedQueryId;
        }
    }
    ...
    public class FindProductsQuery : IQueryLoader<IProduct>
    {
        ...
        public override string ToString()
        {
            ...
            var cachedQueryId = string.Format("products:p_{0}o_{1}-s_{2}-f_{3}", pageSize, offset,
                string.Join(",", searchTokens), filter);
            return cachedQueryId;
        }
    }
