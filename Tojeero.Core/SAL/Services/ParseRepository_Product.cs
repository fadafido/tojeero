...
namespace Tojeero.Core.Services
{
    public partial class ParseRepository
    {
        ...
        public async Task<IProduct> SaveProduct(ISaveProductViewModel product)
        {
            if (product == null || product.Store == null)
                throw new NullReferenceException(
                    "When saving product the ISaveProductViewModel parameter as well as product's store should be non null");
            using (var tokenSource = new CancellationTokenSource(Constants.SaveProductTimeout))
            {
                ...
                p.SearchTokens = new[] {p.Name, p.TagList}.Tokenize();
        ...
        public async Task<Dictionary<string, int>> GetProductSubcategoryFacets(string query,
            IProductFilter filter = null)
        {
        ...
        private ParseQuery<ParseProduct> getFilteredProductQuery(ParseQuery<ParseProduct> query, IProductFilter filter)
        {
            if (filter != null)
            {
                if (filter.Category != null)
                {
                    query =
                        query.Where(
                            p => p.Category == ParseObject.CreateWithoutData<ParseProductCategory>(filter.Category.ID));
                }
                ...
                if (filter.Subcategory != null)
                {
                    query =
                        query.Where(
                            p =>
                                p.Subcategory ==
                                ParseObject.CreateWithoutData<ParseProductSubcategory>(filter.Subcategory.ID));
                }
        ...
        private Query getFilteredProductQuery(Query query, IProductFilter filter, params string[] excludeFacets)
        {
            if (filter != null)
            {
                ...
                var numericFilters = new List<string>();
        ...
        private async Task<Dictionary<string, int>> getProductAttributeFacets(string query, string facetAttribute,
            IProductFilter filter = null, params string[] childFacets)
        {
            var algoliaQuery = new Query(query);
            algoliaQuery = getFilteredProductQuery(algoliaQuery, filter, new[] {facetAttribute}.Concatenate(childFacets));
            algoliaQuery.SetNbHitsPerPage(0);
        ...
        ParseQuery<ParseProduct> addProductVisibilityConditions(ParseQuery<ParseProduct> query)
        {
            var result =
                query.Where(
                    p => p.NotVisible == false && p.Status == (int) ProductStatus.Approved && p.IsBlocked == false);
            return result;
        }
