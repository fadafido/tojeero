...
namespace Tojeero.Core.Services
{
    public partial class ParseRepository : IRestRepository
    {
        ...
        {
            _algoliaClient = new AlgoliaClient(Constants.AlgoliaApplicationId, Constants.AlgoliaSecurityKey, null,
        ...
        public async Task<IEnumerable<ICity>> FetchCities(string countryId)
        {
            ...
            {
                var query =
                    new ParseQuery<ParseCity>().Where(
                        c => c.Country == ParseObject.CreateWithoutData<ParseCountry>(countryId));
                var result = await query.FindAsync(tokenSource.Token).ConfigureAwait(false);
        ...
        public async Task<IEnumerable<IProductSubcategory>> FetchProductSubcategories(string categoryID)
        {
            ...
            {
                var query =
                    new ParseQuery<ParseProductSubcategory>().Where(
                        sub => sub.Category == ParseObject.CreateWithoutData<ParseProductCategory>(categoryID));
                var result = await query.FindAsync(tokenSource.Token).ConfigureAwait(false);
        ...
        private ParseQuery<T> getContainsAllQuery<T, ItemType>(ParseQuery<T> query, string propertyName,
            IList<ItemType> items) where T : ParseObject
        {
            var newItems = items.Count <= Constants.ParseContainsAllLimit
                ? items
                : items.SubCollection(0, Constants.ParseContainsAllLimit);
